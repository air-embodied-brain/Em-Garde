...
from vlm2vec.model.processor import QWEN2_5_VL_TOKENSELECTION
from vlm2vec.arguments import ModelArguments, TrainingArguments
from vlm2vec.model.processor import LLAVA_NEXT, QWEN2_VL, PHI3V, get_backbone_name, print_master, QWEN2_5_VL, \
    backbone2model, QWEN2_VL_TOKENSELECTION, QWEN2_5_VL_TOKENSELECTION, E5_V

from vlm2vec.arguments import ModelArguments
from vlm2vec.model.processor import LLAVA_NEXT, QWEN2_VL, PHI3V, get_backbone_name, print_master, QWEN2_5_VL, INTERNVIDEO2, \
    QWEN2_VL_TOKENSELECTION, backbone2model, GME, VLM_IMAGE_TOKENS, LamRA, LamRA_QWEN2_5, COLPALI
from vlm2vec.model.baseline_backbone.colpali import ColPali
from vlm2vec.model.baseline_backbone.gme.gme_inference import GmeQwen2VL
from vlm2vec.model.baseline_backbone.lamra.lamra_inference import LamRAQwen2VL
from vlm2vec.model.baseline_backbone.lamra.lamra_qwen25_inference import LamRAQwen25VL
...
class MMEBModel(nn.Module):                    # basic class for embedding models
    ...
    def encode_input(self, input):
        if getattr(self, "model_backbone", None) == INTERNVIDEO2:
            ...
            hidden_states = self.encoder(**input, return_dict=True, output_hidden_states=True)  # forward the base model (Qwen2VLForConditionalGeneration)
            end_time = time.time()
    ...
    def build(cls, model_args: ModelArguments, **kwargs):
        ...
        if model_backbone == PHI3V:
            ...
            config.padding_side = "left"
            config.use_cache = False
            base_model = backbone2model[model_backbone].from_pretrained(          # Qwen2VLForConditionalGeneration
                model_args.model_name,
                config=config,
                torch_dtype=torch.bfloat16,
                low_cpu_mem_usage=True,
    ...
    def load(cls, model_args: ModelArguments, is_trainable=True, **kwargs):
        ...
        if model_args.model_backbone in {LLAVA_NEXT, QWEN2_VL, QWEN2_5_VL, QWEN2_VL_TOKENSELECTION, QWEN2_5_VL_TOKENSELECTION, E5_V}:
            ...
            print_master(f'Loading backbone [{model_args.model_backbone}] from {"vlm2vec/model/vlm_backbone/internvideo2/"}')
            config = AutoConfig.from_pretrained("vlm2vec/model/vlm_backbone/internvideo2/",
                                                trust_remote_code=True)
            base_model = backbone2model[model_args.model_backbone].from_pretrained("vlm2vec/model/vlm_backbone/internvideo2/", config=config,
                                                                                   trust_remote_code=True)
        ...
        if model_args.lora:
            ...
            model = cls(
                ...
                normalize=model_args.normalize,
                temperature=model_args.temperature
            ) #This is just a MMEBModel instance, initialized here
        else:
            model = cls(
                encoder=base_model,
                pooling=model_args.pooling,
