...
from vlm2vec.model.vlm_backbone.qwen2_vl import Qwen2VLForConditionalGeneration
from vlm2vec.model.vlm_backbone.qwen2_vl import Qwen2VLProcessor


class LamRAQwen2VL(nn.Module):
    def __init__(
        self,
        model_name: str = "code-kunkun/LamRA-Ret",
        ...
        **kwargs,
        ...
        # self.processor = AutoProcessor.from_pretrained(model_name, size=size, **kwargs)
        self.processor = Qwen2VLProcessor.from_pretrained(model_name, size=size, **kwargs)
    ...
    def embed(self, texts: list[str], images: list[Image.Image], is_query=True, instruction=None, **kwargs):
        # self.base.to(self.device)  # why this?...
        ...
        for t, img_path in zip(texts, images):
            ...
            msg = self.construct_messages(text=t, image=img_path)
...
if __name__ == '__main__':
    ...
    gme = LamRAQwen2VL("Alibaba-NLP/gme-Qwen2-VL-2B-Instruct")
